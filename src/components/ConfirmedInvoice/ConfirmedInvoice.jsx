...
const ConfirmedInvoice = ({def, invoice, number}) => {
  ...
  return def ? (
    <Box  className={`${styles.default_box} ${styles.invoice_box} br6 pad20 grid jcs aic`}>
      ...
      <Typography variant={"h6"}>{t("dashboard.confirmed_invoices.date")}</Typography>

      <Typography variant={"h6"}>{t("dashboard.confirmed_invoices.total_price")}</Typography>
      
      <Typography variant={"h6"}>{t("dashboard.confirmed_invoices.paid_status")}</Typography>
    </Box>
  ):(
    <Box className={`${styles.invoice_box} br6 pad20 grid jcs aic`}>
      ...
      <Typography variant={"h6"}>{handleDate(invoice.updatedAt,t("lang"))}</Typography>

      <Typography variant={"h6"}>{invoice.totalAfterDiscount.toFixed(1)}</Typography>

      <Typography variant={"h6"} sx={{color:(theme)=>`${invoice.isPaid ? theme.palette.whatsapp : theme.palette.youtube}`}} >{invoice.isPaid ? t("dashboard.confirmed_invoices.paid") : t("dashboard.confirmed_invoices.not_paid")}</Typography>
