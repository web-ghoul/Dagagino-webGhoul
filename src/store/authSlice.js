...
export const authSlice = createSlice({
  ...
  reducers: {
    login: (state, action) => {
      ...
      Cookies.set(`${process.env.NEXT_PUBLIC_PROVIDERTYPEID_NAME}`, action.payload.providerTypeId, { expires: 30 })
      state.token = action.payload.token
      state.userId = action.payload.userId
      state.userType = action.payload.userType
      state.userTypeId = action.payload.userTypeId
      state.providerTypeId = action.payload.providerTypeId
      state.signed = true
    },
    logout: (state) => {
      ...
      Cookies.remove(`${process.env.NEXT_PUBLIC_PROVIDERTYPEID_NAME}`)
      state.token = null
      state.userId = null
      state.userType = null
      state.userTypeId = null
      state.providerTypeId = null
      state.signed = false
    },
    setAuth: (state, action) => {
      ...
      state.providerTypeId = action.payload.providerTypeId
      state.token = action.payload.token
      if (action.payload.token && action.payload.userId && action.payload.userTypeId && action.payload.userType && action.payload.providerTypeId) {
        state.signed = true
      }
    }
  }
